...
#define uStar 0.3739
#define Lin -254.5957 /* L at the inlet - L must be < 0 to use this UDF set!!! */
#define Lmast -224.2239 /* L at the mast position Interpolation is performed from the inlety to the mast for the L values so that at the inlet the value is Lin and at the mast the value is L mast */
#define T0 313.0
#define Tstar -0.108
#define ablHeight 800.0
...
#define densOper 1.0827 /* Problem density */
#define Cp 1006.43
#define beta 0.0032
...
/* ********************* Profiles ******************************
********************** Inlet Velocity ********************** */
DEFINE_PROFILE(inletVelocityUnstable,t,i)
{
real x[ND_ND];
real z;
real phiM;
...
F_PROFILE(f, t, i) = (uStar/vonKarman)*(log(8.0*(z/z0) * (pow(phiM,4.0))/(pow(phiM+1.0,2.0)*(pow(phiM,2.0)+1.0))) -pi/2.0 + 2.0*atan(1.0/phiM));
}
end_f_loop(f,t)
}
...
DEFINE_PROFILE(inletTemperatureUnstable,t,i)
{
real x[ND_ND];
real z;
...
phiM = pow(1.0-16.0*(z/Lin),-0.25);
potenTemp = T0 + (Tstar/vonKarman)*(log(z/z0) -2.0*log(0.5*(1.0+pow(phiM,-2.0))));
...
/* ********************* Wall Temperature ********************** */
DEFINE_PROFILE(wallTemperatureUnstable,t,i)
{
real x[ND_ND];
face_t f;
begin_f_loop(f,t)
...
{
real fUn, phiM, phiE, phiH, CkD, source, Gb, Sk, uStarLocal;
real x[ND_ND];
real z, L;
C_CENTROID(x,c,t);
z = C_UDMI(c,t,0) + z0;
...
fUn = 2.0-(z/L) + 8.0*(1.0-12.0*(z/L)+7.0*pow(z/L,2.0)) - 16.0*(z/L)*(3.0-54.0*(z/L)+35.0*pow(z/L,2.0));
CkD = pow(vonKarman,2)/(sigma_k*sqrt(Cmu));
Gb = -C_MU_T(c,t)*pow(sqrt(C_U_G(c,t)[2]*C_U_G(c,t)[2] + C_V_G(c,t)[2]*C_V_G(c,t)[2]),2.0)*((z/L)/(sigma_theta))*(phiH/pow(phiM,2)); /* DTU Formulation */
Sk = pow(uStarLocal,3.0)/(vonKarman*L)*((L/z)*(phiM - phiE) - (phiH)/(sigma_theta*phiM) - 0.25*CkD*pow(phiM,6.5)*pow(phiE,-1.5)*fUn);
...
DEFINE_SOURCE(epsilon_source_Fluent_Unstable,c,t,dS,eqn)
{
real Gb, C3e, source;
...
DEFINE_SOURCE(epsilon_source_AM_Unstable,c,t,dS,eqn)
{
real x[ND_ND];
real z, L;
real Gb, C3e, source;
...
else if (N_ITER > 5 && z/L < -2.3){
Gb = beta*g*C_MU_T(c,t)/PrTurb*C_T_G(c,t)[2];
C3e = -6.523095460000015;
}
else{
Gb = 0.0;
C3e = 0.0;
...
DEFINE_SOURCE(epsilon_source_AM_Unstable_2,c,t,dS,eqn)
{
real x[ND_ND];
real z, L;
...
phiE = 1.0-(z/L);
phiH = sigma_theta*pow(1.0-16.0*(z/L),-0.5);
fe = pow(phiM,2.5)*(1.0-0.75*16.0*(z/L));
/* Gb = beta*g*C_MU_T(c,t)/PrTurb*C_T_G(c,t)[2]; */ /*Standard Fluent Gb formulation, C_MU_T = Turbulent Viscosity, PrTurb = Turbulent Prandtl number, C_T_G = [partial_T/partial_xi] */
...
C3e = (sigma_theta/(z/L))*(phiM/phiH)*(Ce1*phiM-Ce2*phiE+(Ce2-Ce1)*pow(phiE,-0.5)*fe); /* DTU C3e formulation */
...
C_V(c,t) = (uStar/vonKarman)*(log(8.0*(z/z0) * (pow(phiM,4.0))/( pow(phiM+1.0,2.0)*(pow(phiM,2.0)+1.0))) -pi/2.0 + 2.0*atan(1.0/phiM)); /* y velocity */
