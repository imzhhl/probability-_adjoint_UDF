...
/*第一个污染源n1的坐标*/
#define direct_no1_x 0.1
#define direct_no1_y 0.2
#define direct_no1_z 0.2

/*第二个污染源n2的坐标*/
#define direct_no2_x 2.0
#define direct_no2_y 2.0
#define direct_no2_z 2.0

/*第三个污染源n3的坐标*/
#define direct_no3_x 3.0
#define direct_no3_y 3.0
#define direct_no3_z 2.0

/*第一个检测点n1的坐标*/
#define inverse_no1_x 0.3
#define inverse_no1_y 0.3
#define inverse_no1_z 0.3

/*第二个检测点n2的坐标*/
#define inverse_no2_x 0.6
#define inverse_no2_y 0.3
#define inverse_no2_z 0.2

/*第三个检测点n3的坐标*/
#define inverse_no3_x 0.7
#define inverse_no3_y 0.2
#define inverse_no3_z 0.1
...
DEFINE_SOURCE(direct_source_no1, c, t, dS, eqn)
{
	...
	if(x[0]>direct_no1_x - di&&x[0]<direct_no1_x + di&&x[1]>direct_no1_y - di &&x[1]<direct_no1_y + di&&x[2]>direct_no1_z - di&&x[2]<direct_no1_z + di )
	{
		source=100;
	}
	else
...
DEFINE_SOURCE(inverse_source_no1, c, t, dS, eqn)
{
	...
	if(x[0]>inverse_no1_x - di&&x[0]<inverse_no1_x + di&&x[1]>inverse_no1_y - di &&x[1]<inverse_no1_y + di&&x[2]>inverse_no1_z - di&&x[2]<inverse_no1_z + di)
	{
		source=1;
	}
	else
...
DEFINE_SOURCE(inverse_source_no2, c, t, dS, eqn)
{
	...
	if(x[0]>inverse_no2_x - di&&x[0]<inverse_no2_x + di&&x[1]>inverse_no2_y - di &&x[1]<inverse_no2_y + di &&x[2]>inverse_no2_z - di&&x[2]<inverse_no2_z + di)
	{
		source=1;
	}
	else
...
DEFINE_SOURCE(inverse_source_no3, c, t, dS, eqn)
{
	...
	if(x[0]>inverse_no3_x - di&&x[0]<inverse_no3_x + di&&x[1]>inverse_no3_y - di &&x[1]<inverse_no3_y + di&&x[2]>inverse_no3_z - di&&x[2]<inverse_no3_z + di)
	{
		source=1;
	}
	else
...
DEFINE_UDS_FLUX(adjoint_flux, f, t, i)
{
	...
	double NV_VEC(psi_vec), NV_VEC(A), flux = 0.0;
	...
	if (BOUNDARY_FACE_THREAD_P(t)) /*Most face values will be available*/
	{
		...
		flux = -1 * NV_DOT(psi_vec, A); /* flux through Face */
	}
	else
	{
		...
		flux = -1 * NV_DOT(psi_vec, A)/2.0; /* Average flux through face */
	}
	/* ANSYS Fluent will multiply the returned value by phi_f (the scalar’s value at the face) to get the ‘‘complete’’ advective term. */
	return flux;
...
DEFINE_ON_DEMAND(adjoint_probability)
{
	...
	cell_t c, c1, c2, c3; 
	Thread *t, *t1, *t2, *t3; 
	real coord_Cell1[3];					/*找到的cell1的中心坐标 */
	real coord_Cell2[3];					/*找到的cell2的中心坐标 */
	real coord_Cell3[3];					/*找到的cell3的中心坐标 */
	real coord_Source[3];				/*找到的污染源的中心坐标 */
	CX_Cell_Id* cx_cell1 = NULL; 
	CX_Cell_Id* cx_cell2 = NULL; 
	CX_Cell_Id* cx_cell3 = NULL; 
	real Pt_to_find1[3]={inverse_no1_x, inverse_no1_y,inverse_no1_z};		/*指定寻找点1的坐标*/
	real Pt_to_find2[3]={inverse_no2_x, inverse_no2_y,inverse_no2_z};		/*指定寻找点2的坐标*/
	real Pt_to_find3[3]={inverse_no3_x, inverse_no3_y,inverse_no3_z};		/*指定寻找点2的坐标*/
	...
	if (NULL != cx_cell1 || NULL != cx_cell2 || NULL != cx_cell3) 
	{ 
		...
		Message0("coordinate of the specified point: (%g, %g, %g)\n",Pt_to_find1[0],Pt_to_find1[1],Pt_to_find1[2]); 
		Message0("coordinate of the cell found: (%g, %g, %g)\n",coord_Cell1[0],coord_Cell1[1],coord_Cell1[2]);
		Message0("Value of NO1 = %g\n\n",yp1);

  	    Message0("coordinate of the specified point: (%g, %g, %g)\n",Pt_to_find2[0],Pt_to_find2[1],Pt_to_find2[2]); 
		Message0("coordinate of the cell found: (%g, %g, %g)\n",coord_Cell2[0],coord_Cell2[1],coord_Cell2[2]);
		Message0("Value of NO2 = %g\n\n",yp2);

		Message0("coordinate of the specified point: (%g, %g, %g)\n",Pt_to_find3[0],Pt_to_find3[1],Pt_to_find3[2]); 
		Message0("coordinate of the cell found: (%g, %g, %g)\n",coord_Cell3[0],coord_Cell3[1],coord_Cell3[2]);
			...
			thread_loop_c(t, domain)
			{
				...
				{	
					...
					if(C_UDSI(c, t, 2) * C_UDSI(c, t, 4) * C_UDSI(c, t, 6)>= 0.1 || C_UDSI(c, t, 2) * C_UDSI(c, t, 4)>= 0.1 || C_UDSI(c, t, 4) * C_UDSI(c, t, 6)>= 1 || C_UDSI(c, t, 2) * C_UDSI(c, t, 6)>= 0.1 )
					{
						...
						Message0("coordinate of the origial source: (%d, %d)\n",direct_no1_x, direct_no1_y,direct_no1_z); /*TUI输出污染源坐标 */
						Message0("coordinate of the find source: (%g, %g)\n",coord_Source[0], coord_Source[1],coord_Source[2]); /*TUI输出反演源坐标 */
						Message0("Successful! \n");
						goto here; /*跳出所有循环 */
					}
